...
description: >-
  Apply Clean Code principles to reviews and edits: intention-revealing names,
  small functions that do one thing, minimal argument lists, clear error
  handling, clean tests, and boundary isolation around third-party code. Use
  when improving readability, maintainability, refactoring legacy code, or
  creating review checklists. Summarizes Clean Code (Robert C. Martin) as
  practical agent instructions, not a book copy.
---
...
This skill is used for **improving code quality**, not for bike-shedding
formatting. Prefer minimal diffs that reduce cognitive load, improve error
locality, and make future changes cheaper.

## Compatibility

Use for language-agnostic clean-code reviews and refactors. Includes
TypeScript/JavaScript adaptations (Node and Angular). Optional scripts require
Bash and either `rg` (ripgrep) or `grep`.
...
- Do not optimize performance unless requested or the refactor makes it
  strictly better.
...
- **Optimize for reading**: code is read far more than it is written. Make
  "read path" the default.
- **Boy Scout Rule**: always leave the area you touched cleaner than you found
  it.
- **Command-Query Separation**: functions either _do_ or _answer_, not both.
- **Prefer exceptions (or a consistent Result type) over error codes**: keep the
  happy path readable.
- **Do not return null; do not pass null**: prefer empty objects/lists,
  exceptions, or special-case objects.
- **Avoid train wrecks**: long call-chains leak internals; use Law of Demeter
  style boundaries.
- **Formatting is communication**: choose team rules and apply consistently;
  structure files like a newspaper.
- **Clean tests are non-negotiable**: follow TDD loop, keep tests readable; use
  F.I.R.S.T. as the test quality bar.

## Operating principles

### Primary objective

Reduce **WTFs per minute** in code review by making the code read like a story
from top to bottom.
...
- Remove duplication in tests too, but do not DRY them into unreadable
  factories.
...
- Comments are allowed when they explain **why**, constraints, or non-obvious
  tradeoffs.
...
- Create small adapters so the rest of the codebase stays stable if vendors
  change.
...
## Example-driven course map

See `references/ACADEMIND-COURSE-MAP.md` when a review needs practical
before/after anchors for naming, comments, formatting, function parameters,
guard clauses, validation extraction, side effects, unit tests, or object/class
boundaries. Use it as a branch/source map; do not bulk-import course text or
code into the skill.
...
- `scripts/scan-smells.sh` finds common "clean code" smells: TODO/FIXME,
  commented-out code, deep nesting hints, and console noise.
...
- Added an Academind course branch map for practical clean-code lookup without
  bloating the core skill.

### 1.1.0 (2026-02-26)

- Added deeper coverage for readability, functions, formatting, Law of Demeter,
  null handling, tests (TDD + F.I.R.S.T.), and key heuristics.
